...
public class LottieComposition {
  ...
  public static class Factory {
    ...
    public static LottieComposition fromJsonSync(Resources res, JSONObject json) {
      ...
      // 处理https://bugly.qq.com/v2/crash-reporting/crashes/16ccedcc7e/2122?pid=1
      String[] versions = version.split("\\.");
      int major =1,minor=0,patch=0;
      if (versions!=null && versions.length>2) {
        major = Utils.parseInt(versions[0],1);
        minor = Utils.parseInt(versions[1],0);
        patch = Utils.parseInt(versions[2],0);
      }

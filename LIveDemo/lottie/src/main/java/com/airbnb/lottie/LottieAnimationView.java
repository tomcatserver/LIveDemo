...
public class LottieAnimationView extends AppCompatImageView {
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        ...
        animationName = TextUtils.isEmpty(animationName) ? ss.animationName : animationName;
        if (!TextUtils.isEmpty(animationName)) {
            try {
                setAnimation(animationName);
            } catch (Exception e) {
                // f切换横竖屏后会引发fileNotfoundException  bugly 上报id：1217911 。可通过此ID查询具体信息
            }
        }
        ...
        if (!TextUtils.isEmpty(animationName) && animationName.equals(ss.animationName)) {
    ...
    public void setDownloadAnimation(String animationName, String animConfigPath, String version) {
        setDownloadAnimation(animationName, animConfigPath, version, defaultCacheStrategy);
    }
    ...
    public void setDownloadAnimation(final String animationName, final String animConfigPath, final String version, final CacheStrategy cacheStrategy) {
        this.animationName = animationName;
        animationResId = 0;
        if (ASSET_WEAK_REF_CACHE.containsKey(animationName + version)) {
            WeakReference<LottieComposition> compRef = ASSET_WEAK_REF_CACHE.get(animationName + version);
            LottieComposition ref = compRef.get();
            if (ref != null) {
                setComposition(ref);
                return;
            }
        } else if (ASSET_STRONG_REF_CACHE.containsKey(animationName + version)) {
            setComposition(ASSET_STRONG_REF_CACHE.get(animationName + version));
            return;
        }

        lottieDrawable.cancelAnimation();
        cancelLoaderTask();
        File jsonFile = new File(animConfigPath, animationName);
        ...
        compositionLoader = LottieComposition.Factory.fromInputStream(getContext(), fis,
                new OnCompositionLoadedListener() {
                    @Override
                    public void onCompositionLoaded(LottieComposition composition) {
                        if (cacheStrategy == CacheStrategy.Strong) {
                            ASSET_STRONG_REF_CACHE.put(animationName + version, composition);
                        } else if (cacheStrategy == CacheStrategy.Weak) {
                            ASSET_WEAK_REF_CACHE.put(animationName + version, new WeakReference<>(composition));
                        }
